...
gg1<-ggplot(respondents_merged, aes()) + geom_polygon(data = world, aes(x=long, y = lat, group = group), fill = "grey", color="white")+xlab("")+ylab("") + 
  coord_fixed(1.3)



p <- gg1 + 
  geom_point(data = respondents_merged, aes(x = lat, y = long, text = paste(Country,":", percentage, "percentage of survey respondents")), color = "blue", size = respondents_merged$freq/800, alpha = 0.4)+xlab("")+ylab("")
