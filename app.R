...
setwd("C:/Users/amonk/Documents/project")
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      tags$ul(
        ...
        tags$ol(
          ...
          tags$li(a("Developer Roles", href="#Developer_Roles")),
    ...
    mainPanel(
      ...
      #dev roles
      fluidRow(h3(id="Developer_Roles", href="#Developer Roles")),
      fluidRow(h4(id="Developer_Type", href="#Developer Type")),
      
      
      tabsetPanel(
        
        tabPanel("Web Developer Type", plotOutput("web_developer_plot")),
        tabPanel("Mobile Developer Type",plotOutput("mobile_developer_plot")),
        tabPanel("Other Developer Type",plotOutput("other_occupation_plot"))),
      ...
      fluidRow(h3(id="Education", href="#Education")),
      fluidRow(h4("Educational Attainment", href="#Educational Attainment")),
     
       tabsetPanel(
        
        tabPanel("All Respondents", plotOutput("education_all_plot")),
        tabPanel("Professional Developers",plotOutput("education_professional_plot"))),


      
      tags$div(
        tags$p("Worldwide, about three-fourths of professional developers have the equivalent of a bachelor's degree or higher. It is not that rare to find accomplished professional developers who have not completed a degree.")),
      
      #genders - demographics
      fluidRow(h3(id="Demographics", href="#Demographics")),
      fluidRow(h4("Gender", href="#Gender")),
      
      #gender
      tabsetPanel(
        
        tabPanel("All Respondents", plotOutput("gender_all_plot")),
        tabPanel("Professional Developers",plotOutput("gender_professional_plot")),
        tabPanel("Students",plotOutput("gender_student_plot"))),
      
     
      tags$div(
        tags$p("We asked our respondents about their gender identity, and found that over 90% of our respondents are male. According to Quantcast, women account for about 10% of Stack Overflow's US traffic; this year 9% of US survey respondents are women. We had survey participation at almost the rate we would expect from our traffic, although such a low percentage points to problems with inclusion in the tech industry in general and Stack Overflow in particular. In regions including the United States, India, and the UK, women are represented at higher levels among students than among professional developers.")),
      tags$p("This year, 0.7% of respondents identified as transgender men or women. The gender identifications are select all that apply, so transgender men and women are included in the categories shown here."),
      
      
       #race
      fluidRow(h4("Race and Ethnicity", href="#Race and Ethnicity")),
      
      tabsetPanel(
        
        tabPanel("All Respondents", plotOutput("ethnicity_all_plot")),
        tabPanel("Professional Developers",plotOutput("ethnicity_professional_plot")),
        tabPanel("Students",plotOutput("ethnicity_student_plot"))),
 
      
      tags$p("Here again we see evidence for problems with diversity and inclusion. We see higher proportions of developers of color in students than professional developers. This year, 7.4% of professional developers in the United States identified as black, Hispanic or Latino/Latina, or Native American while over 10% of students in the United States identified as a member of one of these groups."),
      
      
      #parent's education
      fluidRow(h4("Parent's Education Level", href="#Parent's Education Level")),
      
      tabsetPanel(
        
        tabPanel("All Respondents", plotOutput("parents_all_plot")),
        tabPanel("Professional Developers",plotOutput("parents_professional_plot")),
        tabPanel("Students",plotOutput("parents_student_plot"))),
      
      
      tags$p("Like developers themselves, most developers' parents have the equivalent of a bachelor's degree or higher. Just under 40% of respondents said their parents do not hold a bachelor's degree."),
...
server <- function(input, output){
  surveyResultsPublic <- read_csv("./survey_results_public.csv")
  
  #Geography - map
  country_codes<- read_csv("./countries.csv")
  #cleaning up data
  #View(country_codes)
  ...
  #plyr
  respondents_count <- plyr::count(respondents, "Country")
  ...
  pro_count <- plyr::count(respondents, "Country")
